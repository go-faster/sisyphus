...
- `internal/convert/anydoc` — `files.Converter` over the [anydoc](https://github.com/firecrawl/anydoc) CLI, run as a **subprocess**, one per document. Off unless `convert.enabled`. Out of process on purpose: it parses untrusted office documents, and a deadline, an output cap and a separate address space are what turn a hang, a runaway allocation or a crash into one lost document instead of a lost run — while keeping every binary `CGO_ENABLED=0`. Conversion is gated on the **extension**, not anydoc's own content sniffing, because the walk decides whether to convert before it reads the file. `.csv` is excluded although anydoc handles it: it is the only supported format that is already text, so it is the only one where enabling the converter would re-index documents that were already fine. Every per-document failure is an `*anydoc.Error` (its `Code` mirrors anydoc's stable variant names, plus `timeout`/`tooLarge`/`crashed`); anything else means the converter itself cannot run, which is why `Available()` is checked once at wiring time rather than per file. No OCR — a scanned PDF is still skipped.
