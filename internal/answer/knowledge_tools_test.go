...
func TestKnowledgeToolSource_SearchKnowledge(t *testing.T) {
	ks := NewKnowledgeToolSource(fakeRetriever{results: []index.Result{{Chunk: index.Chunk{ID: index.NewID(), DocumentID: index.NewID(), Title: "Doc", Text: "body", Type: index.ChunkSection, Metadata: map[string]any{"source": "git", "source_url": "https://example.com/doc"}}, Score: 0.9, Vector: true}}}, fakeFetcher{}, nil, nil, nil)
...
func TestKnowledgeToolSource_FetchURL(t *testing.T) {
	ks := NewKnowledgeToolSource(fakeRetriever{}, fakeFetcher{resp: index.FetchResponse{StatusCode: 200, Body: "ok", FromSite: "site", Truncated: false, Headers: map[string]string{"Content-Type": "text/plain"}}}, nil, nil, nil)
...
func TestKnowledgeToolSource_GetFileContent(t *testing.T) {
	ks := NewKnowledgeToolSource(fakeRetriever{}, fakeFetcher{}, fakeResolver{resp: index.ContentResponse{Content: "package main", Source: "local_clone", Found: true}}, nil, nil)
...
func TestKnowledgeToolSource_Tools_WithResolver(t *testing.T) {
	ks := NewKnowledgeToolSource(fakeRetriever{}, fakeFetcher{}, fakeResolver{}, nil, nil)
	tools, err := ks.Tools(context.Background())
	require.NoError(t, err)
