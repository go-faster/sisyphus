...
- **Conflicts are found by a separate, cursorless sweep** (`conflicts.go`, `Fetcher.FetchConflicts`), because the cursored run structurally cannot see them: an MR starts conflicting when somebody merges into its *target* branch, which leaves its own `updated_at` alone. The sweep lists **open** MRs with `with_merge_status_recheck=true` — list endpoints do not recompute mergeability otherwise, and even with it the recompute is asynchronous, so a `checking` verdict is read as "not conflicting" and settles on the next tick. `has_conflicts` and `detailed_merge_status == "conflict"` are both read; the other blocked statuses (a failing pipeline, a missing approval) are not this event. It emits `event.TypeMRConflict` with its own `ConflictPayload`, not an `MRPayload`: it skips the per-MR discussions and system-notes fetches — that is what makes it cheap enough for its own ticker — and an MR payload with those fields empty would misreport the MR as having no comments and no approvals. The payload carries no timestamp because GitLab reports none; the head `SHA` stands in as the dedup basis. See `internal/notify/CLAUDE.md` § merge conflict.
