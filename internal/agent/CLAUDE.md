...
a single browser snapshot was measured adding ~55k tokens to a 5-call run.
...
The cap is a safety valve, not a quality lever: it is sized to leave an ordinary
`search_knowledge` response (~64KB at limit 30) untouched. Shrinking a chatty tool's own
result is the better fix where one exists.
